...
export default function QuickGameTeamAssignmentPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!roundData.useTeams) {
          setError('Teams are not enabled for this game');
          setLoading(false);
          return;
        }
        ...
        setIsCreator(roundData.createdBy === auth.user.uid);
        ...
        for (const playerId of roundData.players) {
          ...
          // Try to get player handicap from players document
          try {
  ...
  const saveTeams = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'rounds', gameId), {
        teams: {
          team1: {
            name: team1Name,
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="rounded-lg bg-yellow-50 p-6 text-center">
          <h2 className="mb-4 text-xl font-semibold text-yellow-700">Only the game creator can assign teams</h2>
          <Link
            href={`/quick-game/${gameId}`}
            className="inline-block rounded-md bg-green-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-800"
          >
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-600">
          {round?.courseName} - {round?.date && new Date(round.date.toDate()).toLocaleDateString()}
        </p>
      </div>
      ...
      <div className="flex justify-end">
        <button
          onClick={saveTeams}
          disabled={saving}
          className="inline-flex items-center rounded-lg bg-green-600 px-6 py-3 text-base font-medium text-white shadow-md transition-colors hover:bg-green-700 disabled:bg-green-400"
        >
          {saving ? (
            <>
